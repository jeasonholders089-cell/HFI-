...
export async function POST(){
 try{
 const rows=await getDb().select({id:children.id,age:children.age,interests:children.interests,activities:children.activities,selfDescription:children.selfDescription,parentObservation:children.parentObservation,dreamCareer:children.dreamCareer,aiTraits:children.aiTraits}).from(children);
 if(!rows.length)return NextResponse.json({error:'数据库中还没有孩子信息，请先录入。'},{status:400});
 const task=await runInference('anthropic/claude-haiku-4-5',{
 text:JSON.stringify(rows),
 system_prompt:`你是儿童兴趣探索顾问。根据提供的所有学生信息生成活动特质词云。归纳并语义合并相似词，例如观察力强、善于观察、敏锐合为观察敏锐；有创意、想象力强可按具体语义归纳为想象力丰富。最终1至9个简短中文特质，每个词通常4到6字，不要重复近义词，不要为了凑数虚构特质。不作诊断、不作能力定论。根据学生原始自述、活动、兴趣、家长观察找证据，已有aiTraits仅供参考。输出严格JSON：{"groups":[{"word":"观察敏锐","childIds":[1,2]}]}。childIds只能包含输入中真实存在且有该特质依据的学生ID，同一孩子同一特质只算一次，一个孩子可以对应多个特质。不要输出姓名、解释或markdown。`
 });
