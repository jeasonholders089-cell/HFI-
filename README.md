...
```bash
cd HFI-a3e3bf9e          # 应用目录（计划改名为 src/，见文末）
pnpm install
pnpm dev                 # http://localhost:3000
```

本地数据库的连接串在 `HFI-a3e3bf9e/.env`，默认指向本机 Postgres 的 **5434** 端口：
...
本项目应用读的是 `HFI-a3e3bf9e/.env`。这个文件没进版本库，但放在这里会误导人，
后续应当移走。

**应用代码目前还没有纳入 git**，`HFI-a3e3bf9e/` 是未跟踪状态。动手改之前先把这一步补上。
